...
public class BoyaRepository {
    ...
    private static Float parseTemp(String html) {
        try {
            // El HTML contiene: "Agua\n22.7°C" o "Agua</p><p>22.7°C"
            // Buscamos "Agua" seguido de cualquier cosa y luego XX.X°C
            Pattern p = Pattern.compile(
                    "Agua[\\s\\S]{0,50}?(\\d{1,2}[.,]\\d)\\s*°C",
            ...
            // Patrón alternativo: buscar directamente °C precedido de número
            Pattern p2 = Pattern.compile("(\\d{2}[.,]\\d)°C");
            Matcher m2 = p2.matcher(html);
            // Buscar después de "Agua"
            int aguaIdx = html.indexOf("Agua");
            if (aguaIdx >= 0) {
                String sub = html.substring(aguaIdx, Math.min(aguaIdx + 100, html.length()));
                Matcher m3 = p2.matcher(sub);
                if (m3.find()) {
                    String val = m3.group(1).replace(",", ".");
                    return Float.parseFloat(val);
                }
            }
        } catch (Exception e) {
