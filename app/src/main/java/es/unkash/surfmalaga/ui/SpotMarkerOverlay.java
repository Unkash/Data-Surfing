...
public class SpotMarkerOverlay extends Overlay {
    ...
    public void draw(Canvas canvas, MapView mapView, boolean shadow) {
        ...
        for (SurfSpot spot : spots) {
            ...
            // Indicador de bandera encima del círculo
            if (spot.nombreAyuntamiento != null) {
                Paint banderaPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
                banderaPaint.setTextSize(11 * density);
                banderaPaint.setTextAlign(Paint.Align.CENTER);
                String banderaEmoji;
                switch (spot.bandera) {
                ...
                if (!banderaEmoji.isEmpty()) {
                    canvas.drawText(banderaEmoji, p.x, p.y - radius - 4 * density, banderaPaint);
                }
            }

            // Indicador de webcam disponible
            if (spot.webcamUrl != null) {
                Paint camPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
                camPaint.setColor(Color.parseColor("#E53935"));
                camPaint.setTextSize(9 * density);
                camPaint.setTextAlign(Paint.Align.CENTER);
                canvas.drawText("● LIVE", p.x, p.y - radius - 2 * density, camPaint);
            }
        }
    }
